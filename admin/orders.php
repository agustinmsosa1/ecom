...
*/
  ...
  if ($oID && $action && $order_exists) {
    include('includes/classes/order.php');
    $order = new order($oID);
    $languages_query = tep_db_query("select languages_id, name, code, image, directory from " . TABLE_LANGUAGES . " where languages_id = " . $order->info['language_id']);
    $language_dir = tep_db_fetch_array($languages_query);
    $email_language = $language_dir['directory'];
    $comments_flag = tep_image(tep_catalog_href_link('includes/languages/' . $email_language . '/images/' . $language_dir['image'], '', 'SSL'), $language_dir['name']);
  }
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'update_order':
        ...
        // Orders language support BOF
        require('includes/languages/' . $email_language . '/email_constants.php');
        // Orders language support EOF
        $order_updated = false;
        ...
        if ( ($check_status['orders_status'] != $status) || tep_not_null($comments)) {
          ...
          if (isset($_POST['notify']) && ($_POST['notify'] == 'on')) {
            ...
            if (isset($_POST['notify_comments']) && ($_POST['notify_comments'] == 'on')) {
              $notify_comments = sprintf(LOC_EMAIL_TEXT_COMMENTS_UPDATE, $comments) . "\n\n";
            }

            $email = STORE_NAME . "\n" . LOC_EMAIL_SEPARATOR . "\n" . LOC_EMAIL_TEXT_ORDER_NUMBER . ' ' . $oID . "\n" . LOC_EMAIL_TEXT_INVOICE_URL . ' ' . tep_catalog_href_link('account_history_info.php', 'order_id=' . $oID, 'SSL') . "\n" . LOC_EMAIL_TEXT_DATE_ORDERED . ' ' . tep_date_long($check_status['date_purchased'], DATE_FORMAT_LONG_LOCALIZE) . "\n\n" . $notify_comments . sprintf(LOC_EMAIL_TEXT_STATUS_UPDATE, $orders_status_localized_name);

            tep_mail($check_status['customers_name'], $check_status['customers_email_address'], LOC_EMAIL_TEXT_SUBJECT, $email, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
  ...
  <div id="section_status_history_content">
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr class="dataTableHeadingRow">
        ...
        <td class="dataTableHeadingContent" align="center"><strong><?php echo TABLE_HEADING_COMMENTS . '&nbsp;' . $comments_flag;?></strong></td>
