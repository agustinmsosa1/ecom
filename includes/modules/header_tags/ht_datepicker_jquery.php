...
*/
  ...
  class ht_datepicker_jquery {
    ...
    function execute() {
      ...
      if (tep_not_null(MODULE_HEADER_TAGS_DATEPICKER_JQUERY_PAGES)) {
        ...
        if (in_array(basename($PHP_SELF), $pages_array)) {
          ...
          $oscTemplate->addBlock('<script src="ext/datepicker/locales/bootstrap-datepicker.' . JQUERY_DATEPICKER_I18N_CODE . '.min.js" charset="UTF-8"></script>' . "\n", $this->group);
          ...
          // create_account
          // account edit
          $oscTemplate->addBlock('<script>$(\'#dob\').datepicker({language: \'' . JQUERY_DATEPICKER_I18N_CODE . '\', format: \'' . JQUERY_DATEPICKER_FORMAT . '\', startView: 2, autoclose: true,  endDate: now});</script>', $this->group);
          // advanced search
          $oscTemplate->addBlock('<script>$(\'#dfrom\').datepicker({language: \'' . JQUERY_DATEPICKER_I18N_CODE . '\', format: \'' . JQUERY_DATEPICKER_FORMAT . '\', autoclose: true, endDate: now}); </script>', $this->group);
          $oscTemplate->addBlock('<script>$(\'#dto\').datepicker({language: \'' . JQUERY_DATEPICKER_I18N_CODE . '\', format: \'' . JQUERY_DATEPICKER_FORMAT . '\', autoclose: true,  endDate: now});</script>', $this->group);
        }
      }
    }
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_HEADER_TAGS_DATEPICKER_JQUERY_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
    }
